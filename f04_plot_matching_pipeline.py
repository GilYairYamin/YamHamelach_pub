...
class FragmentMatchViewer:
    ...
    def get_matches(
        self,
        sort_by: str = "homo_error_asc",
        min_matches: int = 10,
        max_error: float = 100.0,
        min_error: float = 0.0,
        limit: int = 100,
        validated_only: bool = False,
        valid_homo_only: bool = False,
    ) -> pd.DataFrame:
    ...
    def get_match_details(self, match_id: int) -> Optional[bytes]:
        ...
        try:
            cursor = self.conn.execute(
                "SELECT matches_data FROM matches WHERE id = ?", (match_id,)
            )
    ...
    def get_all_matches_for_fragment(
        self,
        fragment_name: str,
        min_matches: int = 10,
        min_error: float = 0.0,
    ...
    def load_image(self, filepath: str) -> Optional[np.ndarray]:
        ...
        if not os.path.exists(filepath):
            ...
            filepath = os.path.join(
                self.image_base_path, os.path.basename(filepath)
            )
    ...
    def visualize_matches_with_lines(
        self, img1: np.ndarray, img2: np.ndarray, matches_data: bytes
    ) -> np.ndarray:
        ...
        try:
            ...
            sample_matches = (
                random.sample(matches, num_lines_to_draw)
                if len(matches) > num_lines_to_draw
                else matches
            )
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("🔌 Connect", type="primary"):
            ...
            st.session_state.viewer = FragmentMatchViewer(
                db_path, image_base_path
            )
    ...
    if "viewer" not in st.session_state or not st.session_state.get(
        "connected", False
    ):
    ...
    if stats:
        ...
        with col1:
            st.metric(
                "Total Matches", f"{int(stats.get('total_matches', 0)):,}"
            )
        ...
        with col4:
            st.metric("Avg Error", f"{stats.get('avg_homo_error', 0):.2f} px")

        with col5:
            st.metric("Min Error", f"{stats.get('min_homo_error', 0):.2f} px")

    st.divider()

    # Create tabs for different views
    tab1, tab2, tab3 = st.tabs(
        ["📊 Table View & Images", "📈 Analytics", "📉 Error Distribution"]
    )
    ...
    with tab1:
        ...
        if (
            "matches_df" in st.session_state
            and not st.session_state.matches_df.empty
        ):
            ...
            selected_idx = st.selectbox(
                ...
                format_func=lambda x: f"Rank {x + 1}: {matches_df.loc[x, 'file1'][:20]}... ↔ {matches_df.loc[x, 'file2'][:20]}... (Error: {matches_df.loc[x, 'mean_homo_err']:.2f}px)"
                if pd.notna(matches_df.loc[x, "mean_homo_err"])
                else f"Rank {x + 1}: No homography",
            )
            ...
            if selected_idx is not None:
                ...
                with col1:
                    if pd.notna(selected_match["mean_homo_err"]):
                        st.metric(
                            ...
                            delta="Good"
                            if selected_match["mean_homo_err"] < 10
                            else "Poor",
                        )
                    else:
                ...
                with col3:
                    if pd.notna(selected_match["min_homo_err"]) and pd.notna(
                        selected_match["max_homo_err"]
                    ):
                        st.metric(
                            "Error Range",
                ...
                if img1 is not None and img2 is not None:
                    ...
                    with col1:
                        st.image(
                            img1,
                            caption=f"Fragment 1: {selected_match['file1']}",
                    ...
                    with col2:
                        st.image(
                            img2,
                            caption=f"Fragment 2: {selected_match['file2']}",
                            use_container_width=True,
                        )

                    # Show combined visualization
                    if st.checkbox(
                        "Show match visualization with connection lines",
                        value=True,
                    ):
                        match_data = viewer.get_match_details(
                            selected_match["id"]
                        )
                        if match_data:
                            ...
                            st.image(
                                combined,
                                caption="Match Visualization",
                                use_container_width=True,
                            )
                else:
                    st.warning(
                        "⚠️ Could not load one or both images. Please check the image paths."
                    )
                ...
                with col1:
                    ...
                    if st.button(
                        "🖼️ Show complete images",
                        type="secondary",
                        use_container_width=True,
                    ):
                        st.session_state.show_complete_images = True
                        st.session_state.complete_file1 = selected_match[
                            "file1"
                        ]
                        st.session_state.complete_file2 = selected_match[
                            "file2"
                        ]
                ...
                with col2:
                    ...
                    if st.button(
                        "🔍 See all matches of these fragments",
                        type="primary",
                        use_container_width=True,
                        disabled=False,
                    ):
                        st.session_state.show_all_matches = True
                        st.session_state.selected_file1 = selected_match[
                            "file1"
                        ]
                        st.session_state.selected_file2 = selected_match[
                            "file2"
                        ]
                ...
                if st.session_state.get("show_complete_images", False):
                    st.divider()
                    st.subheader(
                        "📸 Complete Images Containing These Fragments"
                    )
                    ...
                    base_file1 = (
                        file1.rsplit("_", 1)[0] + ".jpg"
                        if "_" in file1
                        else file1
                    )
                    base_file2 = (
                        file2.rsplit("_", 1)[0] + ".jpg"
                        if "_" in file2
                        else file2
                    )

                    # Get complete image path from session state
                    complete_path = st.session_state.get(
                        "complete_image_path", ""
                    )

                    if complete_path:
                        # Construct full paths for complete images
                        complete_path1 = os.path.join(
                            complete_path, base_file1
                        )
                        complete_path2 = os.path.join(
                            complete_path, base_file2
                        )
                        ...
                        with col1:
                            if complete_img1 is not None:
                                ...
                                fragment_num1 = (
                                    file1.rsplit("_", 1)[1].replace(".jpg", "")
                                    if "_" in file1
                                    else "N/A"
                                )
                                st.caption(
                                    f"Contains fragment #{fragment_num1}"
                                )
                            else:
                                st.warning(
                                    f"⚠️ Could not load complete image: {base_file1}"
                                )
                        ...
                        with col2:
                            if complete_img2 is not None:
                                ...
                                # Show which fragment this is
                                fragment_num2 = (
                                    file2.rsplit("_", 1)[1].replace(".jpg", "")
                                    if "_" in file2
                                    else "N/A"
                                )
                                st.caption(
                                    f"Contains fragment #{fragment_num2}"
                                )
                            else:
                                st.warning(
                                    f"⚠️ Could not load complete image: {base_file2}"
                                )
                                st.caption(f"Attempted path: {complete_path2}")

                        # Add clear button for complete images
                        if st.button(
                            "❌ Hide complete images",
                        ...
                        st.warning(
                            "⚠️ Complete image path not configured. Please set it in the sidebar."
                        )
                ...
                if st.session_state.get("show_all_matches", False):
                    ...
                    min_match_count = st.session_state.get(
                        "min_matches_for_all", 10
                    )
                    ...
                    with col1:
                        ...
                        matches_file1 = viewer.get_all_matches_for_fragment(
                            ...
                            min_err,
                            max_err,
                            exclude_fragment=file2,
                        )

                        if not matches_file1.empty:
                            st.markdown(
                                f"Found **{len(matches_file1)}** other matches within criteria"
                            )

                            # Display quality distribution
                            quality_counts = matches_file1[
                                "quality"
                            ].value_counts()
                            quality_text = " | ".join(
                                [
                            ...
                            for idx, row in matches_file1.iterrows():
                                ...
                                with st.container():
                                    ...
                                    matched_path = viewer.construct_image_path(
                                        row["matched_fragment"]
                                    )
                                    matched_img = viewer.load_image(
                                        matched_path
                                    )
                    ...
                    with col2:
                        ...
                        matches_file2 = viewer.get_all_matches_for_fragment(
                            ...
                            min_err,
                            max_err,
                            exclude_fragment=file1,
                        )

                        if not matches_file2.empty:
                            st.markdown(
                                f"Found **{len(matches_file2)}** other matches within criteria"
                            )

                            # Display quality distribution
                            quality_counts = matches_file2[
                                "quality"
                            ].value_counts()
                            quality_text = " | ".join(
                                [
                    ...
                    st.divider()
                    if st.button(
                        "❌ Clear all matches view", use_container_width=False
                    ):
    ...
    with tab2:
        ...
        if (
            ...
            valid_errors = matches_df[
                matches_df["mean_homo_err"].notna()
            ].copy()
        ...
        if (
            ...
            if not valid_errors.empty:
                ...
                fig1 = px.histogram(
                    ...
                    labels={
                        "mean_homo_err": "Mean Error (pixels)",
                        "count": "Frequency",
                    },
                ...
                with col2:
                    ...
                    correlation = (
                        valid_errors[["match_count", "mean_homo_err"]]
                        .corr()
                        .iloc[0, 1]
                    )
                    ...
                    if correlation < -0.3:
                        st.info(
                            "Strong negative correlation: More matches tend to have lower errors"
                        )
                    elif correlation < -0.1:
                        st.info(
                            "Weak negative correlation: Slight tendency for more matches to have lower errors"
                        )
                    elif correlation < 0.1:
                        st.info(
                            "No significant correlation between match count and error"
                        )
                    elif correlation < 0.3:
                        st.info(
                            "Weak positive correlation: Slight tendency for more matches to have higher errors"
                        )
                    else:
                        st.info(
                            "Strong positive correlation: More matches tend to have higher errors"
                        )
        else:
            st.info(
                "Load matches in the Table View first to see error distribution"
            )
