...
class NaiveImageMatcher:
    ...
    def _process_image(
        ...
        if image_key in self.cache:
            ...
            return (
                self._deserialize_keypoints(cached_data["keypoints"]),
...
class FragmentMatcher:
    ...
    def get_patch_info(self, file_name, box):
        ...
        json_file = os.path.join(
            self.image_base_path, file_name, f"{file_name}_patch_info.json"
        )
    ...
    def calculate_distances(self, matches_df):
        ...
        for index, row in tqdm(
            ...
            if os.path.exists(image_path1) and os.path.exists(image_path2):
                kp1, kp2, des1, des2, good_matches = self.matcher.calc_matches(
    ...
    def visualize_matches(self, distances, output_dir):
        ...
        for i, match in enumerate(distances):
            ...
            patch1_info = self.get_patch_info(
                os.path.basename(match["file1"]).split("_")[0],
                os.path.basename(match["file1"]).split("_")[1].split(".")[0],
            )
            patch2_info = self.get_patch_info(
                os.path.basename(match["file2"]).split("_")[0],
            ...
            for m in match["matches"]:
                ...
                pt1_orig = (
                    kp1[0] + patch1_info["coordinates"][0],
                    kp1[1] + patch1_info["coordinates"][1],
                )
                pt2_orig = (
                    kp2[0] + patch2_info["coordinates"][0],
                    kp2[1] + patch2_info["coordinates"][1],
                )
    ...
    def write_csv(self, distances, success_csv, missing_csv):
        ...
        with open(missing_csv, mode="w", newline="") as file:
            writer = csv.DictWriter(
                file, fieldnames=["scroll", "fragment", "file1", "file2"]
            )
            writer.writeheader()
    ...
    def run(
        self,
        success_csv="matches.csv",
        missing_csv="missing_files.csv",
        output_dir="match_visualizations",
    ):
        ...
        print(
            f"Results written to {success_csv} and missing file info to {missing_csv}"
        )
