...
class ImagePatchExtractor:
    ...
    def _detect_model_type(self, model_path):
        ...
        if model_path.endswith(".pt") and "yolo" in model_path.lower():
        ...
        elif model_path.endswith(".pth") and (
            "frcnn" in model_path.lower() or "faster" in model_path.lower()
        ):
            ...
            raise ValueError(
                f"Cannot auto-detect model type for {model_path}. Please specify model_type explicitly."
            )
...
def load_args():
    ...
    parser.add_argument(
        "--cp", help="model checkpoint path", default=args.model_nn_weights
    )
    parser.add_argument(
        "--model_type",
