...
if selected_rows:
    ...
    matches_df[['File1', 'Box1']] = matches_df.apply(lambda row: pd.Series(extract_file_box(row['file1'])), axis=1)
    matches_df[['File2', 'Box2']] = matches_df.apply(lambda row: pd.Series(extract_file_box(row['file2'])), axis=1)

    # Filter matches where selected File and Box match
    matches_filtered = matches_df[
        ((matches_df['File1'] == selected_File) & (matches_df['Box1'] == selected_Box)) |
        ((matches_df['File2'] == selected_File) & (matches_df['Box2'] == selected_Box))
        ]
    ...
    if not matches_filtered.empty:
        ...
        gb_matches.configure_selection(selection_mode='single', use_checkbox=True)
