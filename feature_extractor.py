...
if __name__ == "__main__":
    ...
    for label, df in (("true", true_df), ("false", false_df)):
        for i, row in df.iterrows():
            ...
            piece_A = to_patch_fn(
                args.patches, piece_A.split("_")[0], piece_A.split("_")[1]
            )
            piece_B = to_patch_fn(
                args.patches, piece_B.split("_")[0], piece_B.split("_")[1]
            )
            if matcher is None:
            ...
            if features is False:
                print(
                    f"failed to extract features for {piece_A.parts[-1]}, {piece_B.parts[-1]}"
                )
                continue
            ...
            print(
                f"successfully extract features for {label}: {piece_A.parts[-1]}, {piece_B.parts[-1]}"
            )
