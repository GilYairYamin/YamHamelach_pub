...
from f02_SIFT_macher import NaiveImageMatcher, DescriptorCacheManager
...
class HomographyErrorCalculator:
    def __init__(self, matcher: NaiveImageMatcher, error_cache: ErrorCacheManager, descriptor_cache: DescriptorCacheManager):
        self.matcher = matcher
        self.error_cache = error_cache
        self.descriptor_cache = descriptor_cache

    def _compute_homography_and_errors(self, kp1: List[cv2.KeyPoint], kp2: List[cv2.KeyPoint], good_matches: List[cv2.DMatch]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i, m in enumerate(good_matches):
            pt_b_homogenous = np.concatenate((ptsB[i], np.array([1])))  # Convert to homogenous coordinates
            projected_p = H.dot(pt_b_homogenous)
            projected_p_homogenous = projected_p / projected_p[2]  # Normalize by the third (homogeneous) component
    ...
    def calculate_errors_for_pair(self, file1: str, file2: str, good_matches_count: int) -> np.ndarray:
    ...
    with open(csv_file, mode='r') as file:
        ...
        for row in reader:
            if len(row) >= 3:
                file1 = row[0]
                file2 = row[1]
                good_matches_count = int(row[2])  # Get the number of good matches from the CSV
...
if __name__ == "__main__":
    ...
    for file1, file2, good_matches_count in tqdm(read_image_pairs_from_csv(_sift_matches), desc="Calculating Errors"):
        try:
            errors = error_calculator.calculate_errors_for_pair(file1, file2, good_matches_count)
