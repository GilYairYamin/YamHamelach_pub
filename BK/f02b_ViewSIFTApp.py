...
def visualize_match(row, base_path, image_path , patches_key_dec_cache , debug=False):
    ...
    for ii, kep_match in enumerate(sorted_keypoints_matches):
        ...
        # ax3.plot([kp1[0], kp2[0]], [kp1[1], kp2[1]], c='green', alpha=0.5)
        # ax4.plot([kp1[0], kp2[0]], [kp1[1], kp2[1]], c='green', alpha=0.5)
    ...
    #     ax3.scatter(kp[0][0], kp[0][1], s=kp[1] * 10, c='red', marker='x')
    # for kp in keypoints2:
    #     ax4.scatter(kp[0][0], kp[0][1], s=kp[1] * 10, c='red', marker='x')
...
def main():
    ...
    # _sift_matches = os.getenv('SIFT_MATCHES_W_TP')
    # uploaded_file = _sift_matches
    ...
    if input_main_csv_file is not None:
        ...
        if DEBUG == 1:
            ...
            visualize_match(df.iloc[10], PATCHES_IN,  IMAGES_IN_path, patches_key_dec_cache, DEBUG)
        else:
            # Streamlit UI
            ...
            if DEBUG == 2:
                ...
                match_index = 2
                # Select a specific row to visualize
