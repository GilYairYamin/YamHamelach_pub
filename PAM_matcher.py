...
class TwoImagesMatchFeatures:
    ...
    def find_homography(self):
        if (
            len(self._good) < MIN_GOOD_FEATURES_MATCH
        ):  # There is no reason to look for homography..
            return False
    ...
    def to_features(self):
        if hasattr(self, "_errors_hist") is False:
            return False, False
...
class FeatureBasedMatcher(PatchMatcher):
    def __init__(self, fn_1=False, fn_2=False, naive=False):
        self._feature_matcher = TwoImagesMatchFeatures(
            fn_1=fn_1, fn_2=fn_2, naive=naive
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weights", help="weights for NN model", default=model_NN_weights
    )
    ...
    for i, fn_1 in enumerate(patch_path):
        for j, fn_2 in enumerate(patch_path[i + 1 :]):
            ...
            if csv_match and not match:
                ...
                path = Path(
                    args.matches, "miss", f"{fn_1.parts[-1]}-{fn_2.parts[-1]}"
                ).with_suffix(".jpg")
            ...
            if match:
                ...
                m = (
                    fn_1.name,
                    fn_2.name,
                    ["False_pair", "True_pair"][csv_match],
                    *features,
                )
                m = ",".join(m)
                print(m)
